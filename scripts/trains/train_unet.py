...
def get_args():
    ...
    parser.add_argument("--lr", type=float, default=2e-4, help="Learning Rate (менший для U-Net)")
...
def train_unet(args):
    ...
    # 4. Loss & Optimizer (L1 Loss для U-Net)
    criterion = nn.L1Loss()
    ...
    for epoch in range(args.epochs):
        ...
        # Збереження
        if (epoch + 1) % 5 == 0:
            torch.save(model.state_dict(), os.path.join(args.save_dir, f"unet_epoch_{epoch+1}.pth"))

    # Фінальне збереження
    torch.save(model.state_dict(), os.path.join(args.save_dir, "unet_final.pth"))
