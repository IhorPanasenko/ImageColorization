"""
Train Pix2Pix GAN (Stage 3) — improved version.
...
Changes vs original:
  ...
  - Optional warm-start from pretrained U-Net (--warmstart_g)
...
def get_args():
    parser = argparse.ArgumentParser(description="Train Pix2Pix GAN (Stage 3) — Improved")
    # Core
    ...
    parser.add_argument("--warmstart_g", type=str, default=None,
                        help="Pretrained U-Net checkpoint to initialize generator weights (warm-start)")
    # Validation
    ...
    # Fall back to --lr if per-network LR not specified
...
def train_gan(args):
    ...
    print(f"--- STARTING IMPROVED GAN TRAINING on {device} ---")
    ...
    # Optional warm-start: initialize generator from pretrained U-Net
    if args.warmstart_g and os.path.exists(args.warmstart_g):
        ...
        net_G.load_state_dict(ws_state)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.val_every == 0 or epoch == args.epochs - 1:
            val_psnr, val_ssim = validate(net_G, None, val_loader, device)
            ...
            if val_psnr > best_val_psnr:
                ...
                best_ckpt = os.path.join(args.save_dir, "gan_generator_best.pth")
    ...
    print(f"GAN Training Finished! Best val PSNR: {best_val_psnr:.2f} dB")
    if writer:
        writer.close()
