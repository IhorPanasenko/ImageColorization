...
# Resolve src package from scripts/trains/
...
def get_args():
    parser = argparse.ArgumentParser(description="Train Fusion GAN (Stage 4)")
...
def _load_if_exists(model, path, device):
    """Load state dict into model if checkpoint path exists; return parsed epoch."""
...
def train_fusion(args):
    ...
    print(f"--- STARTING FUSION GAN TRAINING on {device} ---")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch in loop:
            ...
            with torch.no_grad():
                global_hint = hint_net(real_L)  # (B, 512)

            fake_ab = net_G(real_L, global_hint)  # (B, 2, 256, 256)
    ...
    print("Fusion GAN Training Finished!")
    if writer:
        writer.close()
